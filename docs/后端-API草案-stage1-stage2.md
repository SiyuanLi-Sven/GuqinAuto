...
- 目前 stage2 仅支持“推荐不写回”：后端返回 top-K 方案，但不会修改 MusicXML 真源。
- `apply_mode="commit_best"`（写回真源）将随 Profile v0.3（引入 `pos_ratio/sound/harmonic_node` 等真值字段）与写回协议稳定后实现；当前若请求写回会明确失败（400），避免“虚假成功”。
