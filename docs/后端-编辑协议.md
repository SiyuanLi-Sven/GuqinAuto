...
- 后端可以提供“计算与建议”（stage1/stage2、一致性检查）。
- **允许写回初稿（写死）**：当项目刚由“简谱（音高+节奏）”初始化且 staff2 尚无任何指法真值时，前端可触发“一键生成初稿”，后端将建议写回生成新 revision（可回滚/可对比）。
- **禁止静默写回（写死）**：除“生成初稿/用户确认提交”这类显式操作外，后端不得在后台悄悄改写 MusicXML 真源。
- 真源写回必须由前端在用户明确操作后调用 `POST /projects/{project_id}/apply`（带 `base_revision`）显式提交（包括：生成初稿与逐单元修改确认）。

补充（实现现状）：
- 后端也提供 `POST /projects/{project_id}/stage2` 的 `apply_mode=commit_best` 作为“生成初稿”的快捷路径：它等价于“先 stage2 推荐，再把 Top-1 显式写回并生成新 revision/delta”。
- 该动作仍属于显式写回（不会在后台静默发生），并会写入 `truth_src=auto,user_touched=0`（见 Profile 文档）。
...
### 4.2.1 上传 MusicXML 创建项目（MVP）

用途：上传 MusicXML 并创建一个 workspace 工程（生成 `R000001`）。

重要约束（学术级）：
- 后端会对上传的 MusicXML 做**严格解析与校验**，要求其可被解析为本项目的事件流（即符合当前 Profile 约定的 `eid` 对齐等规则）。
- 若不符合，必须返回 `400` 并给出明确原因；不做任何“隐式补全/猜测/降级”。

请求：
- `multipart/form-data`
  - `file`：必填，`.musicxml/.xml`
  - `name`：可选，工程名；若不提供则使用文件名（去扩展名）

响应：
- `200 OK`
  - `project`：工程元数据（同 `GET /projects/{project_id}`）
  - `score`：用于前端综合视图的轻量 score view（同 `GET /projects/{project_id}/score`）
...
- `edit_source`：`user`/`auto`，用于后端写回 `GuqinJZP` 的元数据（见 Profile 文档的 `truth_src/user_touched`）
  - `user`：用户编辑确认写回
  - `auto`：导入后“一键生成初稿”写回（系统生成，但属于显式动作，可回滚）
...
- 把 `changes` 里的 key/value 合并进 `GuqinJZP@0.2`/`GuqinJZP@0.3` KV
  - `value=null` 表示**删除该 key**（用于切换 `sound` 时清理互斥字段，如从 `pressed` 切到 `open` 必须删除 `pos_ratio`）
  - 若在 `GuqinJZP@0.2` 上写入了 v0.3 真值字段（如 `sound/pos_ratio/...`），后端会自动升级为 `GuqinJZP@0.3`
